...
`--force` SHALL reinitialize by deleting all target content except the `.git` directory before project-template creation. It SHALL NOT bypass installed-stack validation, project-template type, language compatibility, constraints, strict parsing, or selection ambiguity.
...
### Requirement: CLI project configuration
Successful project-template invocation SHALL also generate `.func/config.json` as CLI-owned output. The file SHALL persist the selected canonical stack and canonical language for every project, including single-language stacks. Project templates SHALL NOT own or generate this file.

#### Scenario: Project is created
- **WHEN** project-template creation succeeds
- **THEN** `.func/config.json` contains the selected canonical stack and language

#### Scenario: Single-language stack is selected
- **WHEN** the selected stack currently supports one language
- **THEN** that canonical language is still persisted explicitly

#### Scenario: Project template generates CLI configuration
- **WHEN** project-template effects include `.func/config.json`
- **THEN** initialization fails the preflight because the path is reserved for the CLI

#### Scenario: Configuration generation fails after project creation
- **WHEN** project files were created but CLI configuration cannot be emitted
- **THEN** the command reports partial initialization and does not delete generated project files

### Requirement: Composite dry-run effects
`func init --dry-run` SHALL perform complete stack, language, template, argument, constraint, and required-value resolution without modifying the filesystem or executing post-actions. The preview SHALL combine project-template effects, CLI-owned `.func/config.json` effects, and `--force` cleanup effects in execution order.

#### Scenario: Empty project is previewed
- **WHEN** a user runs `func init --dry-run` for an empty target
- **THEN** the preview includes project-template files and `.func/config.json` creation without writing either

#### Scenario: Forced project is previewed
- **WHEN** a user combines `--force` and `--dry-run` for a non-empty target
- **THEN** the preview includes non-git deletions followed by project and configuration creation effects

#### Scenario: Template defines post-actions
- **WHEN** the selected project template defines post-actions
- **THEN** the preview reports them without execution

#### Scenario: Project and configuration effects overlap
- **WHEN** project-template and CLI-owned effects target the same reserved path
- **THEN** preflight fails before any filesystem modification

### Requirement: Project invocation effects
Project-template invocation SHALL expose one combined creation-effects result containing project-template and CLI-owned configuration changes. The mechanism used to emit `.func/config.json` MAY be an internal companion template or direct CLI generation with equivalent synthesized effects, provided dry-run and actual invocation remain behaviorally identical.

#### Scenario: Dry-run returns combined effects
- **WHEN** a resolved project template is invoked in dry-run mode
- **THEN** its result includes both project-template and CLI configuration file changes

#### Scenario: Actual invocation returns combined effects
- **WHEN** project-template and CLI configuration generation succeed
- **THEN** the result reports the effects of both operations

#### Scenario: Item template is invoked
- **WHEN** an item-template resolved object is invoked
- **THEN** no project configuration effect is added

### Requirement: Project-template post-actions
After project files and CLI configuration are successfully generated, `func init` SHALL run project-template post-actions by default. Dry-run SHALL never execute them. This capability SHALL NOT define an additional post-action authorization policy.

#### Scenario: Project template has post-actions
- **WHEN** actual project initialization succeeds and the template declares post-actions
- **THEN** the command runs those post-actions in their declared order

#### Scenario: Configuration generation fails
- **WHEN** CLI project configuration cannot be completed
- **THEN** project-template post-actions are not run

#### Scenario: Dry-run has post-actions
- **WHEN** initialization is a dry-run
- **THEN** post-actions are reported but not run
