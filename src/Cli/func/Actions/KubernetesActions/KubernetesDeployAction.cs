...
namespace Azure.Functions.Cli.Actions.KubernetesActions
{
    ...
    internal class KubernetesDeployAction : BaseAction
    {
        ...
        public string HashFilesPattern { get; set; } = string.Empty;
        ...
        public override ICommandLineParserResult ParseArgs(string[] args)
        {
            SetFlag<string>(
                ...
                n =>
                {
                    KubernetesHelpers.ValidateKubernetesName(n);
                    Name = n;
                },
                isRequired: true);
        ...
        public override async Task RunAsync()
        {
            ...
            (var resources, var funcKeys) = await KubernetesHelpers.GetFunctionsDeploymentResources(
                Name,
                resolvedImageName,
                Namespace,
                triggers,
            ...
            if (DryRun)
            {
                ColoredConsole.WriteLine(KubernetesHelpers.SerializeResources(resources, OutputSerializationOptions.Yaml));
            }
            else
            {
                ...
                if (WriteConfigs)
                {
                    var yaml = KubernetesHelpers.SerializeResources(resources, OutputSerializationOptions.Yaml);
                ...
                else
                {
                    ...
                    {
                        var serialized = KubernetesHelpers.SerializeResources(resources, OutputSerializationOptions.Yaml);
                        return KubectlHelper.KubectlApply(serialized, showOutput: true, ignoreError: IgnoreErrors, @namespace: Namespace);
                    };

                    if (!await KubernetesHelpers.NamespaceExists(Namespace))
                    {
                        kubernetesTask = KubernetesHelpers.CreateNamespace(Namespace).ContinueWith((result) =>
                        {
                ...
                {
                    await KubernetesHelpers.WaitForDeploymentRollout(httpDeployment);

                    // Print the function keys message to the console
                    await KubernetesHelpers.PrintFunctionsInfo(httpDeployment, httpService, funcKeys, triggers, ShowServiceFqdn);
                }
            }
        }
        ...
        private async Task<(string Name, bool ShouldBuild)> ResolveImageName()
        {
            var version = "latest";
