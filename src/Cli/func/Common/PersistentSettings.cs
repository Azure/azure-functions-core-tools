...
namespace Azure.Functions.Cli.Common
{
    ...
    {
        private static readonly string s_persistentSettingsPath =
        ...
        public PersistentSettings(bool global)
        {
            if (global)
            {
                FileSystemHelpers.EnsureDirectory(Path.GetDirectoryName(s_persistentSettingsPath));
                _store = DiskBacked.Create<Dictionary<string, object>>(s_persistentSettingsPath);
            }
            else
            {
