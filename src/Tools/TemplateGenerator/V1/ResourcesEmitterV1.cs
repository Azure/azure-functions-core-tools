...
internal static class ResourcesEmitterV1
{
    private const string ResourcesNamespace = "Azure.Functions.Cli.Resources";
    private const string LocalizerName = "KnownResourcesV1Localizer";
    private const string DefaultCulture = "en";

    public static IEnumerable<(string HintName, string Source)> EmitStaticData(ResourcesCatalogModelV1 catalog)
    {
        // Identify the default culture (English) and whether an explicit "en-US" entry already exists.
        ResourceCultureModelV1? defaultCulture = null;
        bool hasEnUs = false;
        foreach (ResourceCultureModelV1 c in catalog.Cultures)
        {
        ...
        // Per-culture dictionary files.
        foreach (ResourceCultureModelV1 culture in catalog.Cultures)
        {
            yield return EmitCulture(culture);
        }

        // Synthetic en-US alias when no explicit Resources.en-US.json file is supplied.
        ...
        // Cultures aggregate + DefaultCulture + TryGetCulture.
        yield return EmitCulturesAggregate(catalog, defaultCulture, emitEnUsAlias);

        // Localizer (only meaningful when we have at least one culture).
    ...
    private static (string HintName, string Source) EmitEnUsAlias()
    {
        ...
        sb.AppendLine("    }");
        sb.AppendLine("}");
        return ("KnownResources.V1.En_US.g.cs", sb.ToString());
    }

    private static (string HintName, string Source) EmitCulture(ResourceCultureModelV1 culture)
    {
        ...
        sb.AppendLine("            {");
        foreach (ResourceEntryModelV1 entry in culture.Entries)
        {
        ...
        sb.AppendLine("    }");
        sb.AppendLine("}");
        return ("KnownResources.V1." + ident + ".g.cs", sb.ToString());
    }

    private static (string HintName, string Source) EmitTokens(ResourceCultureModelV1 defaultCulture)
    {
        var sb = new StringBuilder(8192);
        AppendHeader(sb);
        sb.AppendLine("internal static partial class KnownResources");
        ...
        foreach (ResourceEntryModelV1 entry in defaultCulture.Entries)
        {
        ...
        sb.AppendLine("    }");
        sb.AppendLine("}");
        return ("KnownResources.V1.Tokens.g.cs", sb.ToString());
    }

    private static (string HintName, string Source) EmitCulturesAggregate(ResourcesCatalogModelV1 catalog, ResourceCultureModelV1? defaultCulture, bool includeSyntheticEnUs)
    {
        ...
        sb.AppendLine("        /// <summary>Singleton <see cref=\"global::Microsoft.Extensions.Localization.IStringLocalizer\"/> backed by the generated v1 resource dictionaries.</summary>");
        sb.AppendLine("        public static readonly global::Microsoft.Extensions.Localization.IStringLocalizer Localizer = global::Azure.Functions.Cli.Resources.V1.KnownResourcesV1Localizer.Instance;");
        sb.AppendLine();
        ...
        sb.AppendLine("    }");
        sb.AppendLine("}");
        return ("KnownResources.V1.g.cs", sb.ToString());
    }
    ...
    private static (string HintName, string Source) EmitLocalizer()
    {
        string source = $$"""
            ...
            namespace {{ResourcesNamespace}}.V1;
            ...
            /// <summary>
            /// <see cref="IStringLocalizer"/> implementation backed by the generated v1 resource dictionaries.
            ...
            internal sealed class {{LocalizerName}} : IStringLocalizer
            {
                ...
                private static IEnumerable<IReadOnlyDictionary<string, string>> EnumerateCultureDictionaries(bool includeParentCultures)
                {
                    ...
                    if (includeParentCultures)
                    {
                        ...
                        {
                            if (global::Azure.Functions.Cli.Resources.KnownResources.V1.TryGetCulture(culture, out var dict))
                            {
                                yield return dict;
                            }
                    ...
                    // Always fall back to the default culture last.
                    yield return global::Azure.Functions.Cli.Resources.KnownResources.V1.Default;
                }
            }

            """;
        return ("KnownResourcesV1Localizer.g.cs", source);
    }
