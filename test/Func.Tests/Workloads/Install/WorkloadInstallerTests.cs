...
public sealed class WorkloadInstallerTests : IDisposable
{
    ...
    public async Task InstallFromCatalog_PointerWithoutCurrentRidListsSupportedRids()
    {
        ...
        WorkloadInstaller installer = NewInstaller(metadataReader: new WorkloadMetadataReader());
        WorkloadPackageNotFoundException ex = (await FluentActions.Awaiting(
                () => installer.InstallFromCatalogAsync(
                    ...
                    exact: true,
                    force: false))
            .Should().ThrowExactlyAsync<WorkloadPackageNotFoundException>()).Which;

        ex.Message.Should().Contain("win-x64");
        ex.Message.Should().Contain("linux-x64, osx-arm64");
    }
    ...
    public async Task InstallFromCatalog_MissingPointerImplementationReportsPartialPublish()
    {
        ...
        ex.Message.Should().Contain("partial publish");
        ex.Message.Should().Contain("example.workload.win-x64");
        ex.Message.Should().Contain(resolved.Source.Source);
    }
    ...
    public async Task InstallFromCatalog_ForcedInstall_DoesNotReuseInstalledImplementation()
    {
        ...
        await FluentActions.Awaiting(
                () => installer.InstallFromCatalogAsync(
                    ...
                    exact: true,
                    force: true))
            .Should().ThrowExactlyAsync<WorkloadPackageNotFoundException>();
    ...
    public async Task UpdateLogical_LocalPointerSourceMissing_RequiresExplicitSource()
    {
        ...
        WorkloadInstaller installer = NewInstaller(metadataReader: new WorkloadMetadataReader());
        InvalidOperationException ex = (await FluentActions.Awaiting(
        ...
        ex.Message.Should().Contain("local package");
        await _catalog.DidNotReceive().ResolveLatestVersionAsync(
            Arg.Any<string>(),
            Arg.Any<bool>(),
            Arg.Any<NuGetVersion?>(),
    ...
    public async Task InstallFromPackage_LocalPointerMissingSiblingDoesNotUseCatalog()
    {
        ...
        WorkloadInstaller installer = NewInstaller(metadataReader: new WorkloadMetadataReader());
        FileNotFoundException ex = (await FluentActions.Awaiting(() => installer.InstallFromPackageAsync(pointerPath)).Should().ThrowExactlyAsync<FileNotFoundException>()).Which;

        ex.Message.Should().Contain("missing.workload.win-x64");
        ex.Message.Should().Contain(Path.GetDirectoryName(pointerPath)!);
        ex.Message.Should().Contain("No configured feed");
        await _catalog.DidNotReceive().SearchAsync(
    ...
    public async Task InstallFromPackage_RidImplementationWithOrdinaryPackageTypeIsRejected()
    {
        ...
        WorkloadInstaller installer = NewInstaller(metadataReader: new WorkloadMetadataReader());
        InvalidWorkloadException ex = (await FluentActions.Awaiting(() => installer.InstallFromPackageAsync(packagePath)).Should().ThrowExactlyAsync<InvalidWorkloadException>()).Which;

        ex.Message.Should().Contain(WorkloadInstaller.FuncCliWorkloadRidPackageType);
    }
    ...
    public async Task InstallFromPackage_RidImplementationForDifferentRuntimeIsRejected()
    {
        ...
        ex.Message.Should().Contain("current RID='win-x64'");
        ex.Message.Should().Contain("linux-x64");
    }
